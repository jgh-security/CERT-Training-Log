...
본 사건 분석은 `/var/log/secure` 로그를 중심으로 수행하였다.  
로그 내 SSH 인증 성공 기록(`Accepted password`)을 기준으로 사건의 시작 지점을 식별하였다.

이후 동일 세션에서 발생한 `sudo` 실행 기록을 추적하여  
계정 생성(`useradd intruder`) 및 파일 생성(`touch /tmp/compromised_file`)  
행위를 시간 순으로 상관 분석하였다.

특히 다음과 같은 특징이 확인되었다.

- 동일 Source IP(192.168.200.80)에서 반복적인 SSH 인증 성공 발생
- 로그인 직후 1~2분 이내 sudo 명령 실행
- 신규 계정 생성 및 파일 생성 행위가 연속적으로 기록됨
- 모든 행위가 동일 세션(TTY/pts) 기반에서 수행됨

이를 통해 단순 인증 성공이 아닌,  
시스템 변경을 동반한 침해 활동으로 판단하였다.
