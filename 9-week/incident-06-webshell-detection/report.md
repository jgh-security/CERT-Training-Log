...
또한 특정 파일명(shell.php)에 의존하지 않고,
/hackable/uploads/ 디렉토리 내 .php 파일 접근 자체를 이상 행위로 판단하는 것이 보다 효과적인 탐지 방법이다.
...
단일 키워드(cmd=) 기반 탐지는 오탐 가능성이 있으므로,
경로(/uploads/) 및 파일 확장자(.php)와 함께 조합하여 탐지 정확도를 향상시킬 수 있다.
...
WebShell 명령 실행 패턴을 탐지하기 위해 다음과 같은 Snort 룰을 정의하였다.

```
alert tcp any any -> any 80 (msg:"WebShell Command Execution"; content:"cmd="; http_uri; sid:1000001; rev:1;)
```

해당 룰은 HTTP 요청 URI에 cmd= 파라미터가 포함된 경우 탐지하도록 설정하였다.

---

### 5.3 YARA 룰

WebShell 파일 탐지를 위해 다음과 같은 YARA 룰을 정의하였다.
...
해당 룰은 PHP WebShell에서 자주 사용되는 system 함수와 $_GET 조합을 기반으로 탐지한다.

본 룰은 개념 검증을 위한 예시로 작성되었으며,
실제 환경에서는 추가적인 튜닝 및 검증 과정이 필요하다.
