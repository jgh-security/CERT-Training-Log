...
본 보고서에서는 txt 파일 업로드와 php 파일 업로드를 수행한 뒤, 업로드된 php 파일의 직접 접근 및 실행 여부를 확인하고 nginx access log를 기반으로 공격 흐름을 분석하였다.
...
이는 업로드 후 즉시 파일 실행이 이루어진 흐름을 보여준다.
...
| 시간 | 이벤트 | 근거 로그 | 해석 |
|------|------|------|------|
| 10:25:03 | php 업로드 | POST /vulnerabilities/upload/ | 서버 업로드 요청 |
| 10:28:38 | php 실행 | GET /hackable/uploads/test.php | php 코드 실행 |
...
특히 php 파일 업로드 후 서버에서 실제 실행됨을 확인함으로써 단순 업로드 취약점이 코드 실행으로 이어질 수 있음을 확인하였다.
