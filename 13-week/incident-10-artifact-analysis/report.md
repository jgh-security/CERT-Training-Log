...
```text
touch realbackdoor.sh
chmod +x realbackdoor.sh
```
...
로그인 가능한 계정을 조회한 이후 sudo를 통해 root 권한 상승이 수행되었으며, 이후 realbackdoor.sh 파일 생성 및 실행 권한 부여 행위가 확인되었다.
...
![06_timeline](evidence/screenshots/06_timeline.png)

로그 및 명령어 기록을 기반으로 공격 흐름을 시간 순서대로 재구성하였다.
...
[2026-05-13 14:09:00]
realbackdoor.sh 파일 생성 및 실행 권한 부여 확인
- touch realbackdoor.sh
- chmod +x realbackdoor.sh
...
- 의심 파일(realbackdoor.sh) 생성 행위 확인
