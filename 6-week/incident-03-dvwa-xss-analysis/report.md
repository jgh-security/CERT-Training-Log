...
DVWA의 XSS (Reflected) 페이지(`/vulnerabilities/xss_r/`)에서 다음 payload를 이용하여 취약점을 테스트할 예정이다.

```html id="snr1jt"
<script>alert(1)</script>
```

해당 payload 입력 후 브라우저에서 alert 창이 실행되는지 확인할 예정이다.

이를 통해 사용자 입력값이 HTML 응답에 그대로 반영되는 Reflected XSS 취약 여부를 확인한다.

---

### 3.2 Payload 변형 테스트

기본 script 태그 외에도 이벤트 핸들러 기반 payload를 입력하여 필터링 여부를 확인할 예정이다.

```html id="xahg4j"
<img src=x onerror=alert(1)>
```

해당 payload 실행 여부를 비교하여 단순 script 필터링 우회 가능성을 분석할 예정이다.

---

### 3.3 Stored XSS 테스트

DVWA의 XSS (Stored) 페이지(`/vulnerabilities/xss_s/`)에서 다음 payload를 입력할 예정이다.

```html id="l94y4n"
<script>alert(document.cookie)</script>
```

게시글 저장 후 페이지 재접속 시 저장된 스크립트가 다시 실행되는지 확인하여 Stored XSS 특성을 분석한다.
...
웹 서버의 `/var/log/nginx/access.log` 로그를 분석하여 XSS 관련 요청이 기록되는지 확인할 예정이다.
...
로그 분석을 통해 Source IP, 요청 URI, 입력 payload 반영 여부를 확인할 예정이다.
