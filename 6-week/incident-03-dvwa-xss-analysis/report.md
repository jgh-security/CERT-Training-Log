# Incident 03 - DVWA XSS Attack Analysis

## 1. 사건 개요

본 분석은 DVWA(Damn Vulnerable Web Application) 환경에서 발생 가능한 XSS(Cross-Site Scripting) 공격을 재현하고 웹 서버 로그 기반으로 공격 행위를 분석하기 위한 실습 문서이다.

공격자는 입력값 검증이 수행되지 않는 취약점을 이용하여 JavaScript 코드를 삽입할 수 있으며 브라우저에서 스크립트가 실행되는 현상을 통해 취약 여부를 확인할 수 있다.

본 보고서에서는 Reflected XSS 및 Stored XSS 공격을 단계적으로 수행하고 access log를 기반으로 공격 흐름을 분석할 예정이다.

---

## 2. 분석 환경

| 구분              | 환경                        |
| --------------- | ------------------------- |
| Target          | RHEL                      |
| Attacker        | Kali Linux                |
| Web Server      | nginx                     |
| Web Application | DVWA                      |
| 주요 로그           | /var/log/nginx/access.log |

---

## 3. 공격 재현 과정

### 3.1 Reflected XSS 테스트

DVWA의 XSS (Reflected) 페이지(`/vulnerabilities/xss_r/`)에서 다음 payload를 이용하여 취약점을 테스트하였다.

```html id="2n8g8u"
<script>alert(1)</script>
```

해당 payload 입력 후 브라우저에서 alert 창이 실행되는 것을 확인하였다.

이를 통해 사용자 입력값이 HTML 응답에 그대로 반영되며 브라우저에서 JavaScript 코드가 실행되는 Reflected XSS 취약점이 존재함을 확인하였다.

![Reflected Payload](./evidence/screenshots/01_reflected_payload_input.png)

![Reflected Alert](./evidence/screenshots/02_reflected_alert_success.png)

---

### 3.2 Payload 변형 테스트

기본 script 태그 외에도 이벤트 핸들러 기반 payload를 입력하여 필터링 여부를 확인하였다.

```html id="1p8v8g"
<img src=x onerror=alert(1)>
```

해당 payload 입력 후 브라우저에서 alert 창이 정상적으로 실행되었으며, 존재하지 않는 리소스(`x`)에 대한 추가 요청이 access log에 기록되는 것을 확인하였다.

이를 통해 단순 script 태그 외의 이벤트 기반 payload도 실행 가능함을 확인하였다.

![Onerror Payload](./evidence/screenshots/03_onerror_payload_input.png)

![Onerror Alert](./evidence/screenshots/04_onerror_alert_success.png)

---

### 3.3 Stored XSS 테스트

DVWA의 XSS (Stored) 페이지(`/vulnerabilities/xss_s/`)에서 다음 payload를 입력하였다.

```html id="g9m0ua"
<script>alert(document.cookie)</script>
```

게시글 저장 후 페이지 재접속 시 저장된 스크립트가 반복 실행되었으며 브라우저에서 현재 세션 쿠키(`PHPSESSID`, `security`)가 출력되는 것을 확인하였다.

이를 통해 Stored XSS의 저장 및 반복 실행 특성을 확인하였다.

![Stored Payload](./evidence/screenshots/07_stored_payload_input.png)

![Stored Alert](./evidence/screenshots/08_stored_alert_success.png)

---

## 4. 웹 로그 분석

### 4.1 Access Log 분석

웹 서버의 `/var/log/nginx/access.log` 로그를 분석한 결과 XSS 관련 요청이 정상적으로 기록되는 것을 확인하였다.

Reflected XSS에서는 payload가 GET 요청 URL에 직접 포함되어 access log에서 식별 가능하였으며, Stored XSS에서는 POST 요청이 발생하여 서버 측 저장 구조가 확인되었다.

확인된 주요 패턴은 다음과 같다.

* `%3Cscript%3Ealert%281%29%3C%2Fscript%3E`
* `onerror%3Dalert%281%29`
* `POST /vulnerabilities/xss_s/`

로그 분석을 통해 Source IP, 요청 URI, payload 반영 여부를 확인하였다.

![Access Log Reflected XSS](./evidence/screenshots/05_access_log_xss.png)

![Access Log Onerror](./evidence/screenshots/06_access_log_onerror.png)

![Access Log Stored XSS](./evidence/screenshots/09_access_log_stored_xss.png)

---

### 4.2 공격 단계 분석

로그 패턴을 기반으로 공격 흐름을 분석한 결과 다음 단계의 XSS 공격이 확인되었다.

1. 기본 Reflected XSS 테스트

```html
<script>alert(1)</script>
```

→ 브라우저 내 스크립트 실행 확인

2. 이벤트 핸들러 기반 Payload 테스트

```html
<img src=x onerror=alert(1)>
```

→ 이벤트 기반 payload 실행 및 추가 리소스 요청 확인

3. Stored XSS 등록

```html
<script>alert(document.cookie)</script>
```

→ POST 요청을 통한 payload 저장 및 재실행 확인


---

## 5. 타임라인 재구성

실습 과정에서 확인된 access log를 기반으로 주요 공격 흐름을 시간 순서대로 정리하였다.

| 시간       | 이벤트              | 근거 로그                        | 해석                       |
| -------- | ---------------- | ---------------------------- | ------------------------ |
| 10:08:19 | Reflected XSS 요청 | /vulnerabilities/xss_r/      | 기본 script payload 실행     |
| 10:10:18 | Payload 변형 요청    | /vulnerabilities/xss_r/      | onerror 기반 우회 payload 실행 |
| 10:11:21 | Stored XSS 등록    | POST /vulnerabilities/xss_s/ | 저장형 payload 서버 등록        |

---

## 6. IOC (Indicators of Compromise)

실습 과정에서 확인된 주요 IOC는 다음과 같다.

| 구분        | 값                                         | 설명                    |
| --------- | ----------------------------------------- | --------------------- |
| Source IP | 192.168.200.1                             | 공격 수행 IP              |
| 공격 대상 URL | /vulnerabilities/xss_r/                   | Reflected XSS 페이지     |
| 공격 대상 URL | /vulnerabilities/xss_s/                   | Stored XSS 페이지        |
| 공격 패턴     | `<script>alert(1)</script>`               | 기본 XSS 테스트            |
| 공격 패턴     | `<img src=x onerror=alert(1)>`            | 이벤트 기반 실행             |
| 공격 패턴     | `<script>alert(document.cookie)</script>` | 쿠키 접근 시도              |
| 요청 방식     | GET / POST                                | Reflected / Stored 구분 |
| 로그 위치     | /var/log/nginx/access.log                 | 웹 서버 접근 로그            |

---

## 7. 대응 및 개선 방안

### 7.1 입력값 검증 강화

사용자 입력값에서 script 태그 및 이벤트 핸들러를 필터링해야 한다.

### 7.2 Output Encoding 적용

HTML 출력 시 특수문자를 escape 처리하여 브라우저 실행을 방지해야 한다.

### 7.3 CSP(Content Security Policy) 적용

인라인 스크립트 실행을 제한하여 XSS 피해를 줄일 수 있다.

### 7.4 로그 기반 공격 탐지

다음 문자열을 지속적으로 모니터링할 필요가 있다.

* script
* alert(
* document.cookie
* onerror=

---

## 8. 결론

본 분석에서는 DVWA 환경에서 Reflected XSS 및 Stored XSS 공격을 재현하고 웹 서버 access log를 기반으로 공격 흐름을 분석하였다.

분석 결과 Reflected XSS에서는 GET 요청 URL에 payload가 직접 포함되어 access log에서 식별 가능하였으며, Stored XSS에서는 POST 요청을 통해 payload가 서버 측에 저장되는 구조가 확인되었다.

또한 이벤트 핸들러 기반 payload(onerror)와 쿠키 접근 payload(document.cookie)를 통해 단순 script 태그 외에도 다양한 방식의 JavaScript 실행이 가능함을 확인하였다.

본 사례는 입력값 검증 미흡 및 출력 인코딩 부재 시 XSS 공격이 쉽게 발생할 수 있음을 보여주며, Output Encoding, CSP 적용, 로그 기반 모니터링을 통해 동일 유형의 공격을 예방할 수 있다.





